 ...
 * @version		$Id$
 * @category	Anahita
 * @package		Stonewall Gaming network
 * @copyright	Copyright (C) Stonewall Gaming Network
 ...
 *
 * @category	Anahita
 * @package		Stonewall Gaming Network Plugin
 */
class PlgProfileStonewall extends PlgProfileAbstract
{
	...
	public function __construct($dispatcher, $params)
	{
		...
		JFactory::getLanguage()->load('plg_profile_stonewall', JPATH_ADMINISTRATOR);
	}

	/**
	 * Called on the editing a profile field. Renders all the stonewall fields
	 *
	 * @param  KEvent $config
	 * @return void
	 */
	...
	{	    	    
		...
		$profile = KService::get('repos:stonewall.profile')->findOrCreate(array('person'=>$actor));	
		
		$data = (array) KRequest::get('post.stonewall', 'string');
	...
	public function onDisplay(KEvent $config)
	{
		...
		$actor_profile = KService::get('repos:stonewall.profile')->findOrCreate(array('person'=>$config->actor))->reset();
		$actor_profile->reset();		
	...
	{	
		$osFields = array(
			...
			'GROUP-RELATIONSHIP' => array(
				'relationshipStatus'	=> array('SINGLE','IN-A-RELATIONSHIP','ENGAGED','MARRIED','OPEN'),
